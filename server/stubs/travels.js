...
const travels = [
    ...
    .map(({ name, image: picture, description, continent }) => {
        ...
        const addSevenDays = addDays(10);


        const departure = generateRandomDate(someDaysFromNow, new Date('2024-10-31T23:59:59Z'))

        const twoDaysFromDeparture = addTwoDays(new Date(departure));
        const sevenDaysFromDeparture = addSevenDays(new Date(departure));

        const returnDate = generateRandomDate(twoDaysFromDeparture, sevenDaysFromDeparture)
        ...
        return ({
            ...
            name,
            picture,
            price: Math.floor(400 + Math.random() * 1200),
...
writeFileSync('./server/stubs/travels.json', JSON.stringify(travels), { encoding: 'utf-8' })
