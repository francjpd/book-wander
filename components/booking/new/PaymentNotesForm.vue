    ...
    <div>
        <div class="flex flex-col items-center w-full">
            <form @submit="onSubmit" class="flex flex-col w-80  gap-2 text-xs">
                <FormField v-slot="{ componentField }" :value="internalBooking.paymentType" name="paymentType">
                    <FormItem>
                ...
                <FormField v-slot="{ componentField }" :value="internalBooking.notes" name="notes">
                    <FormItem>
                        <FormLabel>Notes</FormLabel>
                        <FormControl>
                            <Input type="text" placeholder="Notes" v-bind="componentField" />
                        </FormControl>
                        <FormMessage />
                    </FormItem>
                </FormField>
                <Button class="w-80 bg-scooter-400 hover:bg-scooter-600 text-white" type="submit"> Book travel!</Button>
            </form>
        </div>
    </div>
</template>